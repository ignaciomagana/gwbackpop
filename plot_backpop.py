...
plot_backpop.py
---------------
Visualise BackPop posterior results from run_backpop.py.
...
# Human-readable labels — import from backpop, fall back gracefully
try:
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from backpop import PARAM_LABELS, COLS_KEEP, KICK_COLUMNS, BPP_SHAPE, KICK_SHAPE
except ImportError:
    # Fallback labels if backpop.py is not on the path
    PARAM_LABELS = {
        'm1':      r'$m_1\ [M_\odot]$',
        'q':       r'$q_\mathrm{ZAMS}$',
...
def plot_gw_comparison(
    ...
    if samples_path is not None and os.path.exists(samples_path):
        try:
            from backpop import zofdL, ddLdz
